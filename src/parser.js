...
export function parse(source) {
  ...
  const root = { type: 'root', children: [], meta: {} };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Meta declarations (top-level @directives)
...
function parseLine(line) {
  // Match: component "text content" [modifiers...]  key=value...
  ...
  if (!COMPONENT_TYPES.has(keyword)) {
    // Treat as text content if not a known component
    return { type: 'text', content: line };
  }

  const node = { type: keyword, props: {}, styles: [], children: [] };
  ...
  for (const token of tokens) {
    if (token.includes('=')) {
      ...
      // Unknown token - treat as style class
      node.styles.push(token);
    }
  }
...
export { STYLE_KEYWORDS, COMPONENT_TYPES };
